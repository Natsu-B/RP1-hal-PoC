# Selected warm SPI owner (AX initial-mask correction)
...
Initial reset/DONE and exact VERSION/SSIENR/SER/IMR/SR tuple
`[3430322a,0,0,3f,6]` plus quiet NVIC are required before the established
single IMR0 mask write and DSB. Exact posttuple `[3430322a,0,0,0,6]`, then
the broad quiet body/route/NVIC guards are required before setup.
Unknown pretuple rejects without a SPI-body write; unknown posttuple rejects
after only the known mask write. IRQ19/vector35 has priority6 and calls the
...
free-stack guards. E60/61/63..67/6a/6b are preparation refusals; old AW E62
is no longer admitted. E68 is bounded missing
...
watermark. AX161..176 retains pre/post-mask tuples and the post-mask body;
the original pre-mask body remains separate. Watchdog96..123 and fault184..255
remain reserved. ISR/task stack
