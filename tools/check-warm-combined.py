#!/usr/bin/env python3
"""AZ: every ARM, finite owner marker and type8/E edge; no trace filtering."""
import json
...
E, P = A['E'], A['P']
PASS = 'RP1_WATCHDOG_FRESH_PROC0_COMBINED_IRQ_OWNER_SELECTED_PASS'
DIAGNOSTIC = 'RP1_WATCHDOG_WARM_COMBINED_DIAGNOSTIC_CAPTURED'
WIDTHS = (13, 29)
GATE_CODES = (*range(1, 6), *range(0x10, 0x13), *range(0x20, 0x24),
              *range(0x30, 0x35), *(n for n in range(0x40, 0x50) if n != 0x45),
              *range(0x50, 0x55), 0x60, 0x61, 0x63, 0x64, 0x65, 0x66, 0x67,
              0x6a, 0x6b, *range(0x70, 0x7a))


def decode(events, nonce, *, fixed_source_admitted=False):
    assert type(fixed_source_admitted) is bool
    ...
    prefix = events.index(marker[start])
    ...
    common = dict(version=9, trace_events=len(events), marker_edges=len(marker),
                  ...
                  absolute_clock_accuracy_proven=False, warm_combined_owner_proven=False,
                  hardware_reset_entry_proven=False, runtime_restart_proven=False,
                  warm_guard_identity_proven=False)
    if size in (34, 68):
        negative = A['decode'](events, nonce)
        negative.update(common, classification='diagnostic')
        return negative
    assert size in (100, 102, 104), 'incomplete/extra marker or suffix'
    pairs = (size - 100) // 2
    ...
    for i in range(pairs):
        ...
        assert (WIDTHS[i]-1)*1000-100 <= width <= (WIDTHS[i]+1)*1000+100, 'owner pulse width/order'
    ...
    assert kind in (8, 0xe), 'old B/C/D/F/9 is not combined ownership'
    diagnostic = kind == 0xe
    ...
    if not diagnostic:
        assert pairs == 2 and 21_000_000 <= dt < 100_000_000, 'bounded complete owner envelope'
    elif code == 0x78:
        assert 96_000_000 <= dt < 105_000_000, '80-pass diagnostic envelope'
    elif code in (0x77, 0x79):
        assert 16_000_000 <= dt < 105_000_000, 'worker/handoff diagnostic envelope'
        if code == 0x79:
            assert pairs == 2, 'revocation requires completed markers'
    else:
        assert pairs == 0 and 16_000_000 <= dt < 19_000_000, 'preparation diagnostic envelope'
    if hand:
        assert 16_000_000 <= hand[0]['first_timestamp_us'] - arm['frames'][0]['first_timestamp_us'] < 20_000_000
    ...
    return dict(common, classification='diagnostic' if diagnostic else 'HW',
                ...
                warm_combined_owner_proven=not diagnostic and fixed_source_admitted,
...
def validate(text, protocol, before, after, *, fixed_source_admitted=False):
    ...
    assert handoff['before_reload_ncr'] == handoff['pre_ack_ncr'] == 0
    events = P['W3']['W']['R1']['decode_trace'](protocol, before, after)
    ext = decode(events, words[182], fixed_source_admitted=fixed_source_admitted)
    return dict(classification=ext['classification'], result=ext['result'], external=ext,
                handoff=handoff, nonce=words[182], fixed_source_admitted=fixed_source_admitted,
                warm_combined_owner_proven=ext['warm_combined_owner_proven'],
                runtime_restart_proven=ext['runtime_restart_proven'],
                post_ack_registers_read_by_host=False, exact_progress_counters_externally_sampled=False,
                R3='OPEN', boundary='Fixed-source type8 gates two SPI/UART/I2C cycles and fresh R1 progress. Independent native peer/UART payload receipts must join. Not in-flight recovery, health feeding, calibrated timing or PCIe recovery.')
