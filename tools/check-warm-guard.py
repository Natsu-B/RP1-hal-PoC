#!/usr/bin/env python3
"""WDT9 warm-guard diagnostic OR selected fresh-kernel witness, never both."""
import json
...
DIAGNOSTIC='RP1_WATCHDOG_WARM_GUARD_DIAGNOSTIC_CAPTURED'
...
def decode(events,nonce,*,fixed_source_admitted=False):
    ...
    assert kind in (0xc,0xe),'not a WDT9 guard/kernel frame'
    diagnostic=kind==0xe
    ...
    low,high=(16_000_000,19_000_000) if diagnostic else (21_000_000,26_000_000)
...
def validate(text,protocol,before,after,*,fixed_source_admitted=False):
    ...
    return dict(classification=external['classification'],result=external['result'],
        ...
        progress_basis=('Fixed-source gate after five fresh 1s monitor passes, >=5000 ticks/switches, '
        ...
        boundary='E is a warm-guard diagnostic, never kernel restart proof. C is selected fresh proc0 kernel only, conditional on source/ELF/hash admission. No PCIe/peripheral recovery, full R2 recovery, autonomous feeding or general recovery claim.')
