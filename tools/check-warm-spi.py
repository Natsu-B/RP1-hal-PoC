...
def decode(events,nonce,*,fixed_source_admitted=False):
    ...
    low,high=(16_000_000,19_000_000) if diagnostic and entry['raw_reason']!=0x68 else (21_000_000,26_000_000)
    if diagnostic and entry['raw_reason']==0x68:low,high=50_000_000,56_000_000
    ...
    result=dict(common,classification='diagnostic' if diagnostic else 'HW',
        ...
        warm_spi_owner_proven=not diagnostic and fixed_source_admitted)
    if diagnostic:
...
def validate(text,protocol,before,after,*,fixed_source_admitted=False):
    ...
    return dict(classification=external['classification'],result=external['result'],
        ...
        warm_spi_owner_proven=external.get('warm_spi_owner_proven',False),
        ...
        progress_basis=('Fixed-source F gate after two exact4-byte generations from real IRQ19/IPSR35, checked cleanup/canaries/no-rearm, and at least five fresh 1s monitor passes, >=5000 ticks/switches, '
