...
pub const TIMEOUT_US: u32 = 20_000_000;
pub const fn valid_frame(word: u16) -> bool {
    matches!(word, ARMED | ZERO_ALIVE_DISABLED | ERROR)
}
...
pub const fn valid_zero(ctrl: u32, elapsed: u32) -> bool {
    ...
    (ctrl == 0 || ctrl == 0x4000_0000) && elapsed >= 16_000_000 && elapsed < TIMEOUT_US
}
...
mod target {
    ...
    pub unsafe fn emit_pending(marker: &mut ConfiguredPin<22,Output>) -> bool {
        ...
        for iteration in 0..20_000u32 {
            ...
            if valid_zero(ctrl,now.wrapping_sub(start)) {
                put(163,unsafe { read(0x4015_4008) });
                if !unsafe { disable() } { unsafe { error(marker,7,true); } return true; }
                put(164,raw_low());
                let sent = unsafe { emit(marker,ZERO_ALIVE_DISABLED) }; put(166,u32::from(sent));
...
mod tests {
    ...
    fn typed_frames_and_bounded_budget() {
        for word in [ARMED, ZERO_ALIVE_DISABLED, ERROR] {
        ...
        for (ctrl,elapsed) in [(1,16_777_215),(0x8000_0000,16_777_215),
            (0,15_999_999),(0,TIMEOUT_US)] { assert!(!valid_zero(ctrl,elapsed)); }
