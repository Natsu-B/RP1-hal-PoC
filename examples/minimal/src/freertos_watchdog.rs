...
pub const MAGIC: u32 = if cfg!(feature = "freertos-r3-watchdog-warm-guard") { u32::from_le_bytes(*b"WDT9") }
...
pub const VERSION: u32 = if cfg!(feature = "freertos-r3-watchdog-warm-guard") { 9 }
...
pub const REQUEST: u32 = if cfg!(feature = "freertos-r3-watchdog-warm-guard") { u32::from_le_bytes(*b"WQ09") }
    ...
    unsafe fn probe() -> [u32; 12] {
        ...
        if saved == 0 && ctrl == 0 && tick == 3 && cycles == 50 && reason == 2 {
            ...
            unsafe {
                ...
                #[cfg(not(feature = "freertos-r3-watchdog-expiry-entry"))]
